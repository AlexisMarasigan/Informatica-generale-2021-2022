...
namespace game
{
    ...
    public:
        ...
        bool pieRule() const
        {
            ...
            if (!(red_pos.empty()) && (red_pos.size() == 1) && (blue_pos.empty()))
                return true;
            return false;
        }
        ...
        {
            if (turn == 1)
                return board::Piece::RED_DISC;
            if (turn == 2)
                return board::Piece::BLUE_DISC;
        ...
        bool isValid(move::Move m) const
        {
            if (m.kind == move::MoveKind::RESIGN)
                return true;
            if (m.action.piece != getPieceByTurn())
                return false;
            if (m.kind == move::MoveKind::SWAP)
            {
                if (!pieRule())
                    return false;
                if (board.getPieceAtPos(m.action.position) != board::Piece::RED_DISC)
                    return false;
                return true;
            }
            if (board.getPieceAtPos(m.action.position) != board::Piece::EMPTY)
                return false;
            return true;
        }
        ...
        std::vector<move::Move> validMoves() const
        {
            ...
            board::Piece piece = getPieceByTurn();
            for (std::pair<int, int> pos : free_pos)
            {
                move::Action add_action(move::ActionKind::ADD, piece, pos);
        ...
        {
            if (turn < 1)
                throw std::invalid_argument("Invalid Move, the Game is Over.");
            if (!isValid(m))
