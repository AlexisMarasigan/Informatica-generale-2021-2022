...
namespace board {
    ...
    enum class Piece {RED_DISC, BLUE_DISC, EMPTY};
    ...
    class HexBoard {
        ...
        Piece getPieceAtPos(std::pair<int, int> pos) const {
            if (!isValidPos(pos)) throw std::invalid_argument( "Invalid Coordinates for a Position." );   
        ...
        void addPiece(Piece p, std::pair<int, int> pos) {
            if (!isValidPos(pos)) throw std::invalid_argument( "Invalid Coordinates for a Position." );   
            if (getPieceAtPos(pos)!=Piece::EMPTY)  throw std::invalid_argument( "Trying to Add a Piece to an Occupied Position." );   
         ...
         * 
         * @param pos posizione (i,j) in cui cambiare il pezzo
         * @throws InvalidArgumentException se la posizione non è valida per l'attuale scacchiera
         * oppure se la posizione NON è già occupata da un'altra pedina.
         */
        void swapPiece(std::pair<int, int> pos) {
            if (!isValidPos(pos)) throw std::invalid_argument( "Invalid Coordinates for a Position." );   
            if (getPieceAtPos(pos)==Piece::EMPTY)  throw std::invalid_argument( "Trying to Swap a Piece in an Empty Position." );   
            Piece p = getPieceAtPos(pos);
            if (p == Piece::RED_DISC) hex_board.at(pos.first).at(pos.second) = Piece::BLUE_DISC;
            else hex_board.at(pos.first).at(pos.second) = Piece::RED_DISC;
            return;
        }
        ...
        bool checkWin(int color) const {
            if ((color!=1) && (color!=2)) throw std::invalid_argument( "Invalid Color for Winning Check." );              
